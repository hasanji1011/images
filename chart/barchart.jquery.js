		...
		drawY: function(el, options) {
            ...
            while (value <= (stepCount*stepinc)) {
				...
				if (gridValue >= 1000 && gridValue <= 1000000) {
					gridValue = (gridValue / 1000) + ' K';
				}

				if (gridValue >= 1000000 && gridValue <= 1000000000) {
					gridValue = (gridValue / 1000000) + ' M';
				}

				var y = document.createElement('div');

				y.classList.add( yClassName );
				y.style[ yPropertyName ] = top + '%';
...
//			}
		...
		drawX: function(el, options) {
			...
			for (var k in keys) {
				...
				if (keys.hasOwnProperty(k)) {
					...
					barSum.textContent = (localSum.toFixed(2)).toString(); // trim trailing zeros
		...
		subscribe : function(el, options) {
			...
			$barLines.on('mousemove', function(e){
				...
				$tooltip
					.find('.tooltip-change')
					.html( ($currentTarget.data('value').toFixed(2)) + '<small>' + $currentTarget.data('percentage') + '</small>' );
